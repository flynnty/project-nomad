...
function formatZimDisplayName(filename: string): string {
  // youtube_video_VIDEOID_YYYY-MM.zim → "Video: VIDEOID (YYYY-MM)"
  // youtube_channel_CHANID_YYYY-MM.zim → "Channel: CHANID (YYYY-MM)"
  const noExt = filename.replace(/\.zim$/, '')
  const parts = noExt.split('_')
  if (parts.length >= 4) {
    const type = parts[1] // 'video' or 'channel'
    const date = parts[parts.length - 1] + '-' + parts[parts.length - 2]  // YYYY-MM reconstructed
    // Actually the date is already YYYY-MM as the last segment
    const datePart = parts[parts.length - 1]
    const idParts = parts.slice(2, parts.length - 1)
    const id = idParts.join('_')
    return `${type.charAt(0).toUpperCase() + type.slice(1)}: ${id} (${datePart})`
  }
  return filename
}
...
export default function YoutubePage() {
  ...
  // Library of downloaded ZIM files
  const { data: files = [], isLoading: filesLoading } = useQuery<YoutubeFile[]>({
    queryKey: ['youtube-files'],
    queryFn: async () => {
      const res = await api.listYoutubeFiles()
      return res?.files ?? []
    },
    refetchInterval: jobs.some((j) => j.status !== 'failed') ? POLL_INTERVAL_MS : false,
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: (filename: string) => api.deleteYoutubeFile(filename),
    onSuccess: () => {
      addNotification({ type: 'success', title: 'File deleted', message: '' })
      queryClient.invalidateQueries({ queryKey: ['youtube-files'] })
    },
  })
  ...
  return (
    <AppLayout>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        {/* Library */}
        <div>
          <h2 className="text-lg font-semibold mb-3">Downloaded Content</h2>
          {filesLoading ? (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium truncate">{formatZimDisplayName(file.name)}</p>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <a
                      ...
                      className="flex items-center gap-1 text-xs text-desert-green hover:underline"
                      title="Open in Information Library"
                    >
        ...
        <div className="mt-8 p-4 bg-surface-primary border border-desert-stone-light rounded-lg">
          ...
          <ul className="text-sm text-text-secondary space-y-1 list-disc list-inside">
            ...
            <li>Downloaded content is packaged as a ZIM file and automatically appears in the Information Library (Kiwix)</li>
